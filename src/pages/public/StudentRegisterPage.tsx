...
import { User, CreditCard, Lock, BookOpen, ArrowLeft, AlertCircle } from 'lucide-react';
...
export const StudentRegisterPage: React.FC<StudentRegisterPageProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!pin.trim()) {
      setError('Please enter your student PIN (e.g. 24170-CM-001).');
      return;
    }
    ...
    try {
      ...
      if (res.error || !res.user) {
        setError(res.error || 'Student registration failed.');
      } else {
        ...
        <Card className="p-6 sm:p-8 space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-slate-500">
              Department of <span className="font-bold text-indigo-600">{DEPARTMENT}</span> • Register once with your PIN
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <Input
                ...
                helperText="Enter your official college PIN. Teachers will search for this PIN to invite you to classes."
              />
            </div>
